...
const UsersofTransferCredits = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/adminapp/fetchUserData', {
                });
    ...
    return (
        <div style={{ flex: 1 }}>
            ...
            <h4><Badge pill bg="dark">Manage users transfer credit request from here once university gives approval</Badge></h4>
        ...
        <Container fluid>
            <Row xs={1} sm={2} md={3} lg={4} className="g-4">
                {userData.map((user, index) => (
                    <Col key={index}>
                        <Card style={{ width: "100%", margin: '20px 0px 20px 0px' }} className="text-center">
                            <Card.Body>
                                ...
                                <Button variant="primary" onClick={() => handleManageRequestApprove(user.email, user.full_name, navigate)}>Manage Request</Button>
                            </Card.Body>
