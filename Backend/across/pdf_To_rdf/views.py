...
contents_pattern = re.compile(r"Inhalte\s*:\s*(.+?)\s*(?:Qualifikationsziele|\nLehrformen)", re.DOTALL)
...
def extract_information(key, values):
    ...
    if match_contents:
        ...
        result[MODULE_CONTENT] = contents.strip()
...
def get_json(pdf_path, courseName):
    ...
    output_json_path = f'{courseName}.json'
...
# Create RDF graph
    ...
    # Check if work_load is non-empty before converting to integer
        ...
        g.add((module_uri, URIRef("http://tuc/course#hasCourse"), URIRef(f"http://tuc/course#{courseName}")))
...
# Serialize RDF graph to RDF/XML format
    ...
    output_rdf_path = f'{courseName}.rdf'
...
for pdfName in ['WebEngineering', 'SystemEngineering', 'DataScience', 'Informatik', 'Mathematik']:  #'WebEngineering', 'SystemEngineering', 'DataScience', 'Informatik', 'Mathematik'
    ...
    courseName = f'{pdfName}' #replace this with your actual course name
    data = get_json(pdf_path, courseName)
    write_rdf(data, courseName)
